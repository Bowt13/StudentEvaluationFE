...
class BatchCreatorPage extends PureComponent {
  state={
	...
	handleSubmit = (data) => {
    console.log({batchNumber:data.BatchNumber, startDate:data.StartDate, endDate:data.EndDate})
		this.props.createBatch({batchNumber:data.BatchNumber, startDate:data.StartDate, endDate:data.EndDate})
    this.setState({
      open: true,
    })
	}
...
export default connect(mapStateToProps, {createBatch})(BatchCreatorPage)
