...
class StudentViewer extends PureComponent {
	...
	handleSubmit = () => {
		...
		let evaluation = {
			date: new Date(),
			evaluation: currentColorCode,
			remark: Remark,
			student: student,
			teacher: currentTeacher.jwt
		}
	...
	render() {
		...
		let evaluations
		if (student) {
			evaluations = Object.values(student.evaluations).sort(
						...
						return (
							<Paper
								style={{
									...
									width: 50,
									margin: 2,
									backgroundColor: `${this.state.currentState === 'post' &&
										evaluationColor}`,
									textAlign: 'center',
									display: 'inline-block'
								}}
								onClick={_ => this.handleEvaluationClick(evaluation)}
							/>
						)
					})}
				</div>
