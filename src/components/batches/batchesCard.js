  ...
  handleAlgoritm = () => {
    ...
    this.props.students.map((student) => {
      ...
      if(student.batch.id === this.props.batch.id){
        if(student.evaluations.length === 0 && color === 'red') {randomStudents.push(student.id)
        } else {
          if(student.evaluations.length === 0 && color === 'yellow') {
          } else {
            if(student.evaluations.length === 0 && color === 'green') {
            } else {
              if(student.evaluations[student.evaluations.length-1].evaluation === color) randomStudents.push(student.id)
            }
          }
        }
      }
    ...
    let r2 = Math.abs(Math.round(((Math.random() * randomStudents.length) * 0.1) / 0.1))
...
export default connect(mapStateToProps, mapDispatchToProps)(BatchesCard)
